...
use crate::{
    keymap::{initial_define_key_rust, Ctl, KeyChar},
    lisp::LispObject,
    lists::put,
...
pub extern "C" fn keys_of_casefiddle() {
    unsafe {
        // due to use of mutable statics
        initial_define_key_rust(control_x_map, Ctl('U'), "upcase-region");
        put(intern("upcase-region"), Qdisabled, Qt);
        initial_define_key_rust(control_x_map, Ctl('L'), "downcase-region");
        put(intern("downcase-region"), Qdisabled, Qt);

        initial_define_key_rust(meta_map, KeyChar('u'), "upcase-word");
        initial_define_key_rust(meta_map, KeyChar('l'), "downcase-word");
        initial_define_key_rust(meta_map, KeyChar('c'), "capitalize-word");
    }
}
