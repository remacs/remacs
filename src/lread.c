	      ...
	      Lisp_Object start, Lisp_Object end)
{
  ...
  while (continue_reading_p)
	...
	goto read_next;
	  ...
	  || HASH_TABLE_COUNT (XHASH_TABLE (read_objects_map)))
	read_objects_map
	  = make_hash_table (hashtest_eq, DEFAULT_HASH_SIZE,
			     DEFAULT_REHASH_SIZE, DEFAULT_REHASH_THRESHOLD,
			     Qnil, false);
      if (! HASH_TABLE_P (read_objects_completed)
	  || HASH_TABLE_COUNT (XHASH_TABLE (read_objects_completed)))
      ...
      else
	{
      ...
      if (HASH_TABLE_P (read_objects_map)
	  && HASH_TABLE_COUNT (XHASH_TABLE (read_objects_map)) > 0)
	read_objects_map = Qnil;
      if (HASH_TABLE_P (read_objects_completed)
	  && HASH_TABLE_COUNT (XHASH_TABLE (read_objects_completed)) > 0)
